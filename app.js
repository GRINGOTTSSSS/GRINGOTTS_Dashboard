...
async function submitEdit(){
  ...
  try {
    const result = await apiPost({ action:"updateStatus", id:capturedId, status:ns });
    if(result && result.ok) {
      showToast(t("status_ok"),"ok");
      // Don't remove from pending yet — wait for next sync to confirm
    } else {
      showToast(t("status_err"),"err");
    }
  } catch(e) {
    console.warn("Status update error:", e.message);
    showToast(t("status_err"),"warn");
  }
  btn.disabled=false;
}
...
async function addShipment(){
  ...
  // 3. Send to Apps Script
  try {
    const result = await apiPost({ action:"addShipment", shipment:ship });
    if(result && result.ok) {
