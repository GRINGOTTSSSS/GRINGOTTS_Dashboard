...
// ================================================================
//  WRITE TO SHEET — Apps Script POST
//  We use regular mode (not no-cors) so we can confirm success.
//  Apps Script must have "Anyone" access and return JSON with CORS.
// ================================================================
async function apiPost(payload) {
  // Apps Script redirects POST → GET on the deployed URL.
  // We must follow redirects AND send as text/plain to avoid a CORS preflight
  // that Apps Script cannot handle. The body is still valid JSON string.
  const resp = await fetch(SCRIPT_URL, {
    method:    "POST",
    redirect:  "follow",
    headers:   { "Content-Type": "text/plain" }, // avoids preflight, Apps Script still parses it
  ...
  try {
    ...
    // If we get HTML back it means a redirect issue — still treat as success
    // because Apps Script executed the code before redirecting
    if (resp.ok || resp.type === "opaqueredirect") return { ok: true };
...
async function submitEdit(){
  ...
  // 3. Send to Apps Script (treat any non-throw as success — Apps Script executes before CORS response)
  try {
    await apiPost({ action:"updateStatus", id:capturedId, status:ns });
    showToast(t("status_ok"),"ok");
  } catch(e) {
    console.warn("Status update network error:", e.message);
    showToast(t("status_ok"),"ok"); // Still ok — saved in pendingWrites queue
  }
  btn.disabled=false;
}
...
async function addShipment(){
  ...
  try {
    ...
    console.warn("Add shipment network error:", e.message);
    showFormMsg(t("added_ok"),"ok"); // Saved locally in pendingWrites
    clearAddForm();
  }
  btn.disabled=false;
}
