...
// ── WRITE TO SHEET via JSONP (script-tag injection) ──────────────
// This is the ONLY method that reliably works from GitHub Pages to
// Google Apps Script. Fetch with no-cors does NOT reliably trigger
// the Apps Script doGet. A <script> tag is never blocked by CORS.
function callScript(params){
  return new Promise(function(resolve, reject){
    // Build callback name
    var cbName = "gs_cb_" + Date.now() + "_" + Math.floor(Math.random()*9999);

    // Apps Script will call this function with the result
    ...
    var timer = setTimeout(function(){
      cleanup();
      // Resolve anyway — script tag injection is fire-and-forget
      // If we timed out the request was still likely sent
    ...
    var url = SCRIPT_URL + "?" + parts.join("&");

    // Inject script tag — this bypasses all CORS restrictions
