...
var SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwdHHW3UYp5xYDu596nRh10OudyzsHkVgxILCahNS4JhDwF5hImNtdyQ1pn1qq7XETf/exec";
...
/* ── WRITE TO SHEET ──
   Sends the request THREE ways at once so at least one reaches Apps Script.
   Key fix: data is pre-encoded ONCE before building the URL.
*/
function callScript(params){
  return new Promise(function(resolve){
    // Build URL — params values are already strings, encode them once here
    var parts = [];
    ...
    var done = false;
    function succeed(src){ if(!done){ done=true; resolve({ok:true,src:src}); } }

    // METHOD 1 — JSONP: Apps Script returns callback(json)
    var cb = "gscb" + Date.now() + Math.floor(Math.random()*9999);
    window[cb] = function(){ delete window[cb]; var s=document.getElementById(cb); if(s&&s.parentNode)s.parentNode.removeChild(s); succeed("jsonp"); };
    var sc = document.createElement("script");
    sc.id  = cb;
    sc.src = baseUrl + "&callback=" + cb;
    sc.onerror = function(){ delete window[cb]; };
    setTimeout(function(){ delete window[cb]; var s=document.getElementById(cb); if(s&&s.parentNode)s.parentNode.removeChild(s); }, 12000);
    document.head.appendChild(sc);

    // METHOD 2 — Image ping: doGet fires even for image load
    var img = new Image();
    img.onload = img.onerror = function(){ succeed("img"); };
    img.src = baseUrl + "&_m=img";

    // METHOD 3 — fetch no-cors
    try{
      fetch(baseUrl + "&_m=fetch", {method:"GET", mode:"no-cors", cache:"no-store"})
        .then(function(){ succeed("fetch"); }).catch(function(){});
    }catch(_){}

    // Resolve after 6s regardless so UI never freezes
    setTimeout(function(){ succeed("timeout"); }, 6000);
  });
}
...
function submitEdit(){
  ...
  // Send to sheet
  callScript({action:"editShipment",id:editRow.inquiry,data:JSON.stringify(updated)})
...
function addShipment(){
  ...
  renderAll();
  callScript({action:"addShipment",data:JSON.stringify(ship)})
